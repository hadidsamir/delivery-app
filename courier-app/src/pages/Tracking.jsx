...
export default function Tracking() {
  ...
  useEffect(() => {
    ...
    // Escuchar cuando el SW pide la ubicación (app en segundo plano)
    const onSwMessage = (event) => {
      if (event.data.type !== 'REQUEST_LOCATION') return
      const stored = courierRef.current
      if (!stored) return
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => console.log('[SW] GPS error:', err),
  ...
  // ── 2. Detectar app en fondo / frente ────────────────────────────────────
  useEffect(() => {
    const handleVisibility = () => {
      const hidden = document.visibilityState === 'hidden'
      setIsBackground(hidden)
      console.log(hidden ? '[GPS] App en segundo plano → SW activo' : '[GPS] App visible → directo cada 3s')
    }
  ...
  useEffect(() => {
    ...
    courierRef.current = courierData
  ...
  // ── Indicador GPS según estado ────────────────────────────────────────────
  const gpsIndicator = () => {
    if (!tracking) return (
      <div className="rounded-2xl px-4 py-3 flex items-center gap-3 bg-gray-100 border border-gray-200">
    ...
    if (isBackground) return (
      <div className="rounded-2xl px-4 py-3 flex items-center gap-3 bg-yellow-50 border border-yellow-200">
        <span className="relative flex h-3 w-3">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-yellow-400 opacity-75"></span>
          <span className="relative inline-flex rounded-full h-3 w-3 bg-yellow-500"></span>
        </span>
        <div>
          <span className="text-yellow-700 font-semibold text-sm block">GPS activo en segundo plano</span>
          <span className="text-yellow-600 text-xs">Enviando ubicación cada 8 seg via Service Worker</span>
        </div>
      </div>
    )
    ...
    return (
      <div className="rounded-2xl px-4 py-3 flex items-center gap-3 bg-green-50 border border-green-200">
        ...
        <div>
          <span className="text-green-700 font-semibold text-sm block">Compartiendo ubicación (en vivo)</span>
          <span className="text-green-600 text-xs">📡 GPS activo en segundo plano — puedes minimizar la app</span>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-[480px] mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={handleStopTracking} className="text-gray-500 hover:text-gray-700">
      ...
      <div className="max-w-[480px] mx-auto px-4 py-4 space-y-4">
        ...
        {order.items && order.items.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Artículos del pedido</h3>
